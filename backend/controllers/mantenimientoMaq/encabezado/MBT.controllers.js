...
export const postMBT = async(req, res)=>{
    const id_estado= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO MBT(id_maquina, id_creador, id_estado)Values(?, ?, ?)';
...
export const putMBT = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE MBT SET id_estado = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
