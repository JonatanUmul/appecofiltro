...
export const postMTA = async(req, res)=>{
    const id_estado= 2;
    ...
    try{
       ...
       const consulta='INSERT INTO mta(id_maquina, id_creador, id_estado)Values(?, ?, ?)';
...
export const putMTA = async (req, res) => {
    ...
    try {
        if (estado === '' || id === '') {
            ...
            const consulta = 'UPDATE MTA SET id_estado = ?, fechaCierre = ?, horaCierre = ? WHERE id = ?';
